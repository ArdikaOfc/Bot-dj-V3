...
let handler = async (m, { conn, text, usedPrefix, command, isOwner, isAdmin }) => {
    ...
    if (command === 'cap') {
        ...
        if (m.quoted) {
            ...
            // Mengambil nomor dari teks (mencari angka yang diawali 62 atau nomor lainnya)
            let mnd = text.split(' ')[0].replace(/[^0-9]/g, '')
            if (mnd.length > 5) { // Validasi sederhana panjang nomor
        ...
        // Mengambil teks cap (menghapus tag atau nomor dari teks)
        let capText
        if (m.quoted) {
            capText = text
        } else {
            // Menghapus tag atau nomor dari input teks
