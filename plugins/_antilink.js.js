...
export async function before(m, { isAdmin, isBotAdmin }) {
    ...
    let isChannelLink = channelRegex.exec(m.text)
    ...
    } else if (chat.antiLinkChannel && m.isGroup) {
        if (isChannelLink && !isAdmin) {
            
            
                m.reply(`_*‼️Link Channel Terdeteksi!*_ ${chat.pembatasan ? '\n_pesan kamu akan di hapus! ❌_': '\n_pesan kamu akan dihapus! ❌_'} ${isBotAdmin ? '': '\n\n_❬Bot Bukan Admin❭_'}`)
