...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        let sewaData = JSON.parse(readFileSync(pathSewa))
        let now = Date.now()
        let changed = false
        for (let i = 0; i < sewaData.length; i++) {
            if (now > sewaData[i].expired) {
                ...
                try {
                    await this.sendMessage(groupID, { text: '⚠️ Masa sewa grup ini telah habis! Bot akan keluar otomatis.\n\nHubungi owner untuk memperpanjang.' })
                    await this.groupLeave(groupID)
                } catch (e) {
                    console.error(`Gagal keluar dari grup ${groupID}`)
                }
                sewaData.splice(i, 1)
                changed = true
                i--
    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (typeof user.totalchat !== 'object') user.totalchat = {}
            } else {
                global.db.data.users[m.sender] = {
                    exp: 0,
                    limit: 100,
            ...
            if (chat) {
                ...
                if (!('nsfw' in chat)) chat.nsfw = fals
        ...
        if (m.isGroup && !m.isBaileys) {
            let user = global.db.data.users[m.sender]
            if (!user.totalchat) user.totalchat = {}
            if (!user.totalchat[m.chat]) user.totalchat[m.chat] = 0
            user.totalchat[m.chat] += 1
        }
        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin !== 'function') continue
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    let chat = global.db.data.chats[m.chat]
                    let user = global.db.data.users[m.sender]
                    if (name != 'owner-unbanchat.js' && name != 'owner-exec.js' && name != 'owner-exec2.js' && chat?.isBanned) return
    ...
    if (chat.welcome) {
        ...
        for (let user of participants) {
            ...
            try {
                ...
                let bg = 'https://c.termai.cc/i177/ztVn8tc'
